...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$result) {
        $_SESSION['toast_error'] = "Tài khoản <b>$email</b> không tồn tại!";
        header('Location: ..\auth\login.php');
        exit();
    }

    if ($result['is_active'] == 0) {
        $_SESSION['toast_error'] = "Tài khoản <b>$email</b> chưa được kích hoạt!";
        header('Location: ..\auth\login.php');
        exit();
    }
    if($result['email_verified_at'] == null) {
        $_SESSION['toast_error'] = "Tài khoản <b>$email</b> chưa được xác thực!";
        header('Location: ..\auth\login.php');
        exit();
    }
    ...
    if (password_verify($password, $result['password'])) {
        if ($result['role'] == 'admin') {
            ...
            $_SESSION['toast_error'] = "Bạn không có quyền truy cập vào trang này!";
            header('Location: ..\auth\login.php');
            exit();
        }
    } else {
...
<head>
    ...
    <title>Đăng nhập - Phòng trọ thành phố Vinh</title>
    <style>
        .bg-pattern {
    ...
    <div class="w-full max-w-md relative">
        ...
        <div class="card login-card w-full shadow-2xl">
            <div class="card-body p-8">
                <h2 class="card-title text-2xl font-bold text-center justify-center mb-4">Đăng nhập</h2>
                <form action="../auth/login.php" method="POST">
                    <div class="form-control">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="email" placeholder="Nhập tên đăng nhập hoặc email" class="input input-bordered w-full pl-10 rounded-3xl" required />
                        </div>
                    </div>
                    ...
                    <div class="form-control mt-6">
                        <button type="submit" name="login" value="1" class="btn btn-primary rounded-3xl">
                ...
                <p class="text-sm text-center mt-6 text-gray-600">
                    ...
                    Chưa có tài khoản? <a href="../auth/register.php" class="text-primary font-medium hover:underline" onclick="return confirm('Đây là trang quản trị viên, bạn chắc chắn là nhân viên quán này chứ?')">Đăng ký ngay</a><br>
                </p>
            </div>
        </div>
